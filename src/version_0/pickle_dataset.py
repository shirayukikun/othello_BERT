...
class PickleIterableDataset(IterableDataset):
    ...
    def __iter__(self):            
        with self.tokenized_data_file_path.open(mode="rb") as f:
            while True:
                try:
                    yield pickle.load(f)
                except EOFError:
...
class PickleDataset(Dataset):
    def __init__(self, dataset_path:Path):
        ...
        assert len(self.tokenized_data) == self.basic_info["number_of_data"], \
            "Number of data dose not match..."
